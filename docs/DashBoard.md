...
### 节点信息查看
在节点列表界面选择某个节点条目点击其中的“查看”按钮进入该节点的信息查看界面。分别能查看节点基本信息、配置信息、密钥对、监控以及日志等信息。
在“基本项”选项卡可查看节点基本信息如节点标识、管控服务Url以及监控服务Url 等；在“配置信息”选项卡可查看该节点程序的具体配置信息；在“密钥对”选项卡可查看下载该节点的JKS密钥对文件和公钥证书文件；在“监控”选项卡可实时查看该节点的系统监控指标（CPU、内存、磁盘、网络等），在左上角下拉框可选择数据的时间窗口（最近 5 分钟、15 分钟、1 小时、24 小时、7 天）, 这里要求节点填写了有效的"监控服务 Url"；在“日志”选项卡可实时查看该节点程序的运行日志，与监控面板一致可在左上角下拉框选择最近 5 分钟至 7 天时间范围，在搜索框中输入关键词后可过滤日志内容（输入停顿约 0.6 秒后自动生效，无需手动确认）

![look-1](images/look-1.png)

![look-2](images/look-2.png)

![look-3](images/look-3.png)

![look-4](images/look-4.png)

![look-5](images/look-5.png)
...
### 前端菜单权限管理
管理员可在账户列表界面选择某个账户项目点击"查看"按钮，选择“操作权限”选项卡，继续选择“前端菜单权限”子选项卡，点击具体要授予或回收的菜单权限开关可对该账户登录后能够使用的前端菜单进行权限管理控制， 如控制该账户对密钥、账户、交易、区块及智能合约等功能菜单项的可访问权限。

![authorize-10](images/authorize-10.png)
...
第三方平台成员创建即账户管理中的新建功能。从“账户管理”中“账户”模块进入，可点击“新建”建立新的账户即新的第三方平台成员，具体信息包括账户标识、账户名称、账户状态等。第三方平台成员创建如下图所示。

![mem-create-1](images/mem-create-1.png)

### 第三方平台成员邀请

第三方平台成员邀请功能支持管理员根据需要填写邀请信息并生成对应的邀请表单，用于向第三方平台发起成员加入邀请。从“账户管理”中“账户”模块进入，点击“创建邀请表单”。选择邀请对象账户类型，并配置需要对方实体提供给运维方操作员的信息项。确认后将生成并下载邀请表单文件。
邀请账户类型分为一般账户和节点账户。以一般账户为例，需要填写基础信息和证书信息。对方需要提供基础信息和证书信息。基础信息包括账户标识、社会信用代码、账户名称、账户手机号等；证书信息包括证书名称、选择证书等。第三方平台成员邀请如下图所示。

![mem-invate-1](images/mem-invate-1.png)


### 成员资格审核
成员资格审核由管理员对提交的成员申请或邀请反馈进行审核处理。从“账户管理”中“账户”模块进入，点击“新建”，管理员根据表单内容及相关条件，对成员资格进行确认，填写账户标识、社会信用代码、账户名称、账户手机号、账户身份证书等。

![mem-check-1](images/mem-check-1.png)

### 成员权限管理
成员权限管理对应账户管理中的前端权限管理功能，主要用于控制不同成员在系统中的可见菜单、可访问页面及可操作功能范围。从“账户管理”中“账户”模块进入，点击“详情”，管理员可根据成员角色和职责分配相应权限，确保其仅能使用被授权的功能内容。成员权限管理如下图所示。

![mem-limt-1](images/mem-limt-1.png)


### 成员退出管理
成员退出管理中，当第三方平台成员不再参与相关协同工作，或因管理需要停止其使用权限时，管理员可通过禁用方式进行处理。从“账户管理”中“账户”模块进入，点击“编辑”，填写账户标识、账户名称，选择账户状态为“禁用”。点击“保存”即可完成成员的退出管理。成员退出管理界面如下图所示。

![mem-qute-1](images/mem-qute-1.png)


### 第三方平台协同机制
第三方平台协同机制设置主要包括治理协同机制和数据协同机制两个方面。在治理协同机制上，系统可基于已有能力进行节点协同角色分配，这一部分可依托现有的相关功能实现。在数据协同机制上，结合已经实现的账户可见菜单权限配置，对不同账户开放不同的数据与功能范围，从而实现数据可见性配置，满足平台间协同管理需求。

### 第三方平台规则设置
第三方平台规则设置可结合邀请生成过程进行体现。从“账户管理”中“账户”模块进入，点击“邀请表单”。根据实际情况填写基础信息、证书信息、节点基本信息、节点配置邀请等，可实现不同场景下的规则配置。第三方平台规则设置如下图所示。

![mem-rute-1](images/mem-rute-1.png)


### 投票结果统计与展示
投票结果统计与展示围绕该笔交易所在区块的节点投票结果进行统计和展示，反映各节点在相关业务处理中的参与情况和投票状态。从“账户管理”中“账户”模块进入，点击“详情”。展示内容包括投票节点、投票结果及整体统计情况等，用于帮助用户直观了解区块确认过程中的共识情况。投票结果统计与展示如下图所示。


![mem-vote-1](images/mem-vote-1.png)
